...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $recipes_blog_url The remote URL.
		 */
		public function __construct( $recipes_blog_url = '' ) {
			$this->remote_url = $recipes_blog_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $recipes_blog_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $recipes_blog_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $recipes_blog_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $recipes_blog_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $recipes_blog_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $recipes_blog_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $recipes_blog_format = 'woff2' ) {
			$this->font_format = $recipes_blog_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $recipes_blog_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $recipes_blog_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $recipes_blog_url, '/' ) ) {
				$recipes_blog_parsed_url = parse_url( $this->remote_url );
				return $recipes_blog_parsed_url['scheme'] . '://' . $recipes_blog_parsed_url['hostname'] . $recipes_blog_url;
			}

			return $recipes_blog_url;
		}
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $recipes_blog_url    The URL of the remote webfont.
	 * @param string $recipes_blog_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_styles( $recipes_blog_url, $recipes_blog_format = 'woff2' ) {
		$recipes_blog_font = new WPTT_WebFont_Loader( $recipes_blog_url );
		$recipes_blog_font->set_font_format( $recipes_blog_format );
		return $recipes_blog_font->get_styles();
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_url' ) ) {
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_url( $recipes_blog_url, $recipes_blog_format = 'woff2' ) {
		$recipes_blog_font = new WPTT_WebFont_Loader( $recipes_blog_url );
		$recipes_blog_font->set_font_format( $recipes_blog_format );
		return $recipes_blog_font->get_url();
	}
}